...
**本仓库已自带参考实现：** 根目录下 **`.cursor/agents/*.md`**（13 个角色 + `README.md`）。以本仓库为工作区打开时，可在 Agent 输入框使用 **`/orchestrator`**、**`/market-analyst`** 等调用（与 [官方说明：Explicit invocation](https://cursor.com/docs/subagents) 一致）。与 `.claude/agents/` 同名时，Cursor **优先**加载 `.cursor/agents/`。
...
官方 **Best practices** 提醒：避免几十个含混 Subagent；本技能包固定 **13 个角色**，属于「数量可控、职责清晰」——请为每个 Subagent 写**高质量 `description`**，否则主 Agent 难以稳定自动委派（见官方 *Invest in descriptions*）。
